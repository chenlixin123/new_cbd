<template>
  <div class="box">
    ...
    <div class="tab_left" v-if="!loading">
      <button class="button_two button_one">整体概览</button>
      <button class="button_two" @click="two">车场列表</button>
      <button class="button_two" @click="three">实时地图</button>
    </div>
    <img class="imgs" src="~@/assets/images/tab_left.png" alt v-if="!loading" />
    <!-- <input class="button_two" type="button" value="车场列表" /> -->
  </div>
</template>
...
export default {
  ...
  created() {
    this.url =
      "https://datav.aliyuncs.com/share/6a19d9ac1e5c91c61f27dec3abef0c1e?timestamp=" +
      new Date().getTime();
  },
  methods: {
    two() {
